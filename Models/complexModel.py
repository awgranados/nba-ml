...
def predict_player_points(player_stats_file, player_averages_file):
    ...
    # Prepare to predict the next game
    # Use the last available game's statistics for prediction
    last_game_stats = mergedDF.iloc[-1][features]  # Get the most recent game statistics
    pred_stats = pd.DataFrame([last_game_stats])  # Convert to DataFrame
